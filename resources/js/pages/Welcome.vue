...
const primaryCta = computed(() => {
  ...
  return { label: "Join the community", href: route("register"), icon: "sparkles" };
});

const secondaryCta = computed(() => {
  if (isAuthed.value) {
    return { label: "View dashboard", href: route("dashboard"), icon: "layout" };
  }
  return { label: "Sign in", href: route("login"), icon: "log-in" };
});

// Placeholder images - in a real app these would be assets
const heroBackground =
  "https://images.unsplash.com/photo-1441974231531-c6227db76b6e?q=80&w=2560&auto=format&fit=crop"; // Nature background
const heroImage =
  "https://images.unsplash.com/photo-1580436541340-29807187421f?q=80&w=2787&auto=format&fit=crop"; // Hibiscus
const potdImage =
  "https://images.unsplash.com/photo-1572454591674-2739f30d8c40?q=80&w=2787&auto=format&fit=crop"; // Bougainvillea
const plant1 =
  "https://images.unsplash.com/photo-1616428790326-7248e301297e?q=80&w=2835&auto=format&fit=crop"; // Rafflesia equivalent/exotic
const plant2 =
  "https://images.unsplash.com/photo-1596727040409-775b9f936f4d?q=80&w=2835&auto=format&fit=crop"; // Torch Ginger equivalent
const plant3 =
  "https://images.unsplash.com/photo-1596727147705-56a53272d560?q=80&w=2835&auto=format&fit=crop"; // Pitcher plant equivalent
...
<template>
  ...
  <div
    class="min-h-screen bg-gradient-to-b from-white via-white to-emerald-50/40 text-slate-950 font-sans selection:bg-emerald-200 selection:text-slate-950"
  >
    <a
      href="#main"
    ...
    <div class="pointer-events-none fixed inset-0 -z-10">
      <div
        class="absolute -top-32 -left-40 h-[420px] w-[420px] rounded-full bg-emerald-300/25 blur-3xl"
      />
      <div
        class="absolute top-24 -right-48 h-[520px] w-[520px] rounded-full bg-indigo-300/15 blur-3xl"
      />
      <div
        class="absolute bottom-[-140px] left-1/3 h-[520px] w-[520px] rounded-full bg-teal-300/15 blur-3xl"
      />
    </div>
    ...
    <header class="sticky top-0 z-40 border-b border-black/5 bg-white/70 backdrop-blur">
      <div class="mx-auto flex w-full max-w-7xl items-center justify-between px-6 py-4">
        <Link
          :href="route('home')"
          class="flex items-center gap-2 rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/40"
        >
          <span
    ...
    <main id="main">
      ...
      <section class="relative isolate overflow-hidden">
        <img
          :src="heroBackground"
          alt=""
          class="absolute inset-0 -z-10 h-full w-full object-cover opacity-[0.08]"
        />
        <div class="mx-auto max-w-7xl px-6 py-12 lg:py-20">
          <div class="grid gap-12 lg:grid-cols-2 lg:items-center">
            <div class="space-y-8">
              <div
                class="inline-flex items-center gap-2 rounded-full border border-emerald-600/15 bg-emerald-600/10 px-3 py-1 text-xs font-semibold text-emerald-800"
              >
              ...
              <div class="space-y-4">
                ...
                >
                  Discover and identify Malaysian plants — with a dashboard built for
                  focus.
                </h1>
                <p class="max-w-xl text-lg leading-relaxed text-slate-600">
                  Turn photos into knowledge, sightings into maps, and curiosity into
              ...
              <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                <Link :href="primaryCta.href">
                  <Button
                    class="h-11 w-full gap-2 rounded-full bg-slate-900 text-white hover:bg-slate-800 sm:w-auto"
                  >
                ...
                <Link v-if="secondaryCta" :href="secondaryCta.href">
                  <Button
                    variant="outline"
                    class="h-11 w-full gap-2 rounded-full sm:w-auto"
                  >
              ...
              <dl class="grid gap-4 pt-2 sm:grid-cols-3">
                <div
                  v-for="s in stats"
                  :key="s.label"
                  class="rounded-2xl border border-black/5 bg-white/70 p-4 shadow-sm"
                >
                  <dt
                  ...
                  <dd class="mt-2 text-2xl font-bold tracking-tight text-slate-950">
                    {{ s.value }}
                  </dd>
                </div>
              </dl>
            ...
            <div class="relative">
              <div
                class="overflow-hidden rounded-3xl border border-black/5 bg-white shadow-xl"
              >
                <img
                  v-if="!imgFailed.hero"
                  :src="heroImage"
                ...
                >
                  <div class="text-center">
                    <div
                      class="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-white/70 text-emerald-700 shadow-sm ring-1 ring-black/5"
                    >
              ...
              <div
                class="absolute bottom-5 left-5 rounded-2xl border border-black/5 bg-white/90 px-4 py-3 shadow-lg backdrop-blur"
              >
                <div class="flex items-center gap-3">
                  <span
                    class="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-600/10 text-emerald-700"
                  >
                    <Icon name="leaf" class="h-5 w-5" />
                  </span>
                  <div>
      ...
      <section class="mx-auto max-w-7xl px-6 py-14">
        <div class="mx-auto max-w-2xl text-center">
          ...
          <p class="mt-3 text-slate-600">
            Three fast workflows, designed with clear next steps and low friction.
          </p>
        </div>
        ...
        <div class="mt-10 grid gap-6 lg:grid-cols-3">
          <Card
            v-for="f in features"
            :key="f.title"
            class="relative overflow-hidden border-black/5 bg-white/70 shadow-sm backdrop-blur"
          >
            <Link
              :href="f.href"
              class="absolute inset-0 z-10 rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/40"
              :aria-label="
                f.locked ? `Create an account to unlock ${f.title}` : `Open ${f.title}`
              "
            >
            ...
            <CardHeader class="relative">
              <div class="mb-3 flex items-center justify-between gap-3">
                <div
                  class="flex h-12 w-12 items-center justify-center rounded-xl bg-emerald-600/10 text-emerald-700"
                >
        ...
        >
          <div>
            ...
            <p class="mt-2 text-slate-600">
              A curated species spotlight with key facts and conservation context.
            </p>
          </div>
          ...
          <div class="grid md:grid-cols-2">
            <div class="relative">
              ...
              >
                <div class="text-center">
                  <div
                    class="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-white/70 text-emerald-700 shadow-sm ring-1 ring-black/5"
                  >
              ...
              <div
                class="absolute left-5 top-5 inline-flex items-center gap-2 rounded-full border border-black/5 bg-white/90 px-3 py-1 text-xs font-semibold text-emerald-800 backdrop-blur"
              >
            ...
            <div class="p-8 md:p-10">
              ...
              <h3 class="mt-2 text-3xl font-bold tracking-tight text-slate-950">
                Paperflower
              </h3>
        ...
        <div class="grid gap-6 md:grid-cols-3">
          ...
          >
            <Link
              ...
              class="absolute inset-0 z-10 rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/40"
              :aria-label="
                isAuthed ? `Browse plants` : `Create an account to browse plants`
              "
            >
            ...
            <CardContent class="pt-6">
              ...
              <p
                class="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-emerald-800"
              >
    ...
    <footer class="border-t border-black/5 bg-white/60 px-6 py-16 backdrop-blur">
      <div class="mx-auto max-w-7xl">
        <div
          class="grid gap-10 rounded-3xl border border-black/5 bg-gradient-to-br from-emerald-600/10 via-white to-indigo-600/10 p-8 shadow-sm md:grid-cols-2 md:items-center md:p-12"
        >
          <div class="space-y-4">
            ...
            <p class="text-slate-600">
              Start identifying, logging sightings, and building your personal dashboard
              today.
            </p>
          </div>
        ...
        <div
          class="mt-10 flex flex-col items-start justify-between gap-4 border-t border-black/5 pt-8 text-sm text-slate-500 sm:flex-row sm:items-center"
        >
